...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Check if there is a specific ID in the URL (e.g., ?id=7)
    const urlParams = new URLSearchParams(window.location.search);
    const id = parseInt(urlParams.get('id'));

    if (id) {
        showDetail(id, false); // false means "don't add to history again"
    }
});
...
window.addEventListener('popstate', (event) => {
    if (event.state && event.state.id) {
        showDetail(event.state.id, false);
    } else {
        showHome(false);
    }
});

// 3. Load the Grid List
function loadList() {
    ...
    data.forEach(item => {
        ...
        card.className = 'card';
        // When clicking, we now pass 'true' to update the URL
        card.onclick = () => showDetail(item.id, true); 
...
function showDetail(id, updateUrl = true) {
    ...
    if (!item) return; // Stop if ID doesn't exist

    // Update URL if requested (only when clicking, not when loading from link)
    if (updateUrl) {
